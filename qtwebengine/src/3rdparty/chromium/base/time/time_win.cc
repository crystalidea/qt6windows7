...
LiveTicks LiveTicksNowIgnoringOverride() {
  ...
  // Windows 7 backport: QueryUnbiasedInterruptTimePrecise() only exists from
  // Windows 10 on and is imported through api-ms-win-core-realtime-l1-1-1.dll,
  // which is not present on Windows 7. Resolve it at run time and fall back to
  // QueryUnbiasedInterruptTime(), which has been in kernel32 since Windows 7
  // and reports the same unbiased interrupt time in the same units of 100
  // nanoseconds, only at timer-tick rather than interpolated resolution.
  using QueryUnbiasedInterruptTimePreciseFn = VOID(WINAPI*)(PULONGLONG);
  using QueryUnbiasedInterruptTimeFn = BOOLEAN(WINAPI*)(PULONGLONG);
  struct Resolved {
    QueryUnbiasedInterruptTimePreciseFn precise = nullptr;
    QueryUnbiasedInterruptTimeFn coarse = nullptr;
  };
  static const Resolved resolved = []() {
    Resolved result;
    // The precise variant is exported by KernelBase.dll, the coarse one by
    // kernel32.dll; both modules are always loaded already.
    if (HMODULE kernelbase = ::GetModuleHandle(L"kernelbase.dll")) {
      result.precise = reinterpret_cast<QueryUnbiasedInterruptTimePreciseFn>(
          ::GetProcAddress(kernelbase, "QueryUnbiasedInterruptTimePrecise"));
    }
    HMODULE kernel32 = ::GetModuleHandle(L"kernel32.dll");
    if (!result.precise && kernel32) {
      result.precise = reinterpret_cast<QueryUnbiasedInterruptTimePreciseFn>(
          ::GetProcAddress(kernel32, "QueryUnbiasedInterruptTimePrecise"));
    }
    if (kernel32) {
      result.coarse = reinterpret_cast<QueryUnbiasedInterruptTimeFn>(
  ...
  if (resolved.precise) {
    resolved.precise(&unbiased_interrupt_time);
  } else if (resolved.coarse) {
    resolved.coarse(&unbiased_interrupt_time);
  } else {
    // Neither is available: report biased time rather than a zero clock.
    unbiased_interrupt_time = ::GetTickCount64() * 10000ULL;
  }
