...
// Windows 7 backport: ::PrefetchVirtualMemory() is only available on Windows 8
// and above, so check for the function's presence dynamically as Chromium did
// up to 109.0.5414.120. Where it is missing PreReadFileSlow() is used, which
// warms the same pages through a plain sequential read.
