...
// Windows 7 backport: SetThreadInformation() was introduced in Windows 8, so
// resolve it at run time as Chromium did up to 109.0.5414.120. Both features it
// is used for here — thread memory priority and EcoQoS power throttling — do
// not exist on Windows 7, and both call sites already treat failure as an
// expected outcome.
decltype(&::SetThreadInformation) GetSetThreadInformationFunction() {
  ...
  return set_thread_information_fn;
}
...
void SetCurrentThreadPriority(ThreadType thread_type,
  ...
  if (g_background_thread_normal_memory_priority_win &&
    ...
    const auto set_thread_information_fn = GetSetThreadInformationFunction();
...
void SetCurrentThreadQualityOfService(ThreadType thread_type) {
  ...
  [[maybe_unused]] const BOOL success =
      set_thread_information_fn &&
      set_thread_information_fn(::GetCurrentThread(), ::ThreadPowerThrottling,
