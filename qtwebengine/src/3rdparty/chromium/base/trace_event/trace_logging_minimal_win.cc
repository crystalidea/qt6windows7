...
ULONG TlmProvider::Register(const char* provider_name,
  ...
  *reinterpret_cast<uint16_t*>(provider_metadata_) = provider_metadata_size_;
  ...
  // Windows 7 backport: EventSetInformation() was introduced in Windows 8, so
  // resolve it at run time. Where it is missing the provider traits simply are
  // not registered, which is exactly the "best-effort" outcome this call
  // already tolerates; EventRegister()/EventWrite() keep working.
  const auto event_set_information_ptr =
  ...
  return event_set_information_ptr(reg_handle_, EventProviderSetTraits,
