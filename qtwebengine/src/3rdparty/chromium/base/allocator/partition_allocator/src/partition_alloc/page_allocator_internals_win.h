...
void DiscardSystemPagesInternal(uintptr_t address, size_t length) {
  void* ptr = reinterpret_cast<void*>(address);
  // Windows 7 backport: DiscardVirtualMemory() was introduced in Windows 8.1.
  // Resolve it at run time so the import does not keep the library from
  // loading; where it is missing the MEM_RESET path below is used, which is
  // what Windows 7 always used and which makes the same guarantees.
  ...
  if (discard_virtual_memory)
    ret = discard_virtual_memory(ptr, length);
