...
ThreadGroup::GetScopedWindowsThreadEnvironment(WorkerEnvironment environment) {
  ...
  if (environment == WorkerEnvironment::COM_MTA) {
    // Windows 7 backport: the version check is restored from Chromium
    // 109.0.5414.120. ScopedWinrtInitializer calls RoInitialize(), which lives
    // in combase.dll and therefore cannot work before Windows 8 - so on
    // Windows 7 every thread pool worker that asks for an MTA would end up with
    // no apartment initialised at all, and anything on those threads that needs
    // COM fails from then on. Initialising the MTA the classic way through
    // CoInitializeEx() gives those workers exactly what they asked for, which
    // is what Chromium did on these systems.
