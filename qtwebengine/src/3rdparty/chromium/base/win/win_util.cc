...
bool IsWindows10OrGreaterTabletMode(HWND hwnd) {
  ...
  // Windows 7 backport: go through the base::win wrapper, which resolves the
  // WinRT entry point out of combase.dll at run time instead of importing it.
...
bool IsUser32AndGdi32Available() {
  static auto is_user32_and_gdi32_available = []() {
    // Windows 7 backport: GetProcessMitigationPolicy() was introduced in
    // Windows 8. Resolve it at run time; where it is missing, win32k syscalls
    // cannot be disabled at all, so user32 and gdi32 are always available.
    const auto get_process_mitigation_policy_func =
...
void EnableHighDPISupport() {
  ...
  // Windows 7 backport: SetProcessDpiAwareness() is exported by shcore.dll,
  // which only exists from Windows 8.1 on, so resolve it at run time. When it
  // is missing the predecessor below is used, which is all Windows 7 supports.
  PROCESS_DPI_AWARENESS process_dpi_awareness = PROCESS_PER_MONITOR_DPI_AWARE;
  const auto set_process_dpi_awareness_func = []() {
    HMODULE shcore =
        ::LoadLibraryEx(L"shcore.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    return shcore ? reinterpret_cast<decltype(&::SetProcessDpiAwareness)>(
                        ::GetProcAddress(shcore, "SetProcessDpiAwareness"))
                  : nullptr;
  }();
  if (!set_process_dpi_awareness_func ||
      !set_process_dpi_awareness_func(process_dpi_awareness)) {
