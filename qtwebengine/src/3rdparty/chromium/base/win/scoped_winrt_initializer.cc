...
// Windows 7 backport: RoInitialize()/RoUninitialize() live in combase.dll,
// which does not exist on Windows 7. Resolve them at run time, as Chromium did
// up to 109.0.5414.120. When they are missing the initializer simply fails,
// which is a state every caller already handles through Succeeded().
FARPROC LoadComBaseFunctionForWinrtInitializer(const char* function_name) {
...
decltype(&::RoInitialize) GetRoInitializeFunction() {
  static decltype(&::RoInitialize) const function =
      reinterpret_cast<decltype(&::RoInitialize)>(
          LoadComBaseFunctionForWinrtInitializer("RoInitialize"));
  return function;
}
...
decltype(&::RoUninitialize) GetRoUninitializeFunction() {
  static decltype(&::RoUninitialize) const function =
      reinterpret_cast<decltype(&::RoUninitialize)>(
          LoadComBaseFunctionForWinrtInitializer("RoUninitialize"));
  return function;
}
