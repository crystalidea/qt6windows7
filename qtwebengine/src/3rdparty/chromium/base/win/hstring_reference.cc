// Copyright 2019 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "base/win/hstring_reference.h"

#include <windows.h>

#include <wchar.h>
#include <winstring.h>

#include "base/check_op.h"
#include "base/numerics/safe_conversions.h"

namespace base {
namespace {

// Windows 7 backport: WindowsCreateStringReference() is exported by
// combase.dll, which does not exist on Windows 7. Resolve it at run time, as
// Chromium did up to 109.0.5414.120, so the import does not keep the library
// from loading. On Windows 8 and later the same function is called as before.
decltype(&::WindowsCreateStringReference) GetWindowsCreateStringReference() {
  static auto const create_string_reference_func =
      []() -> decltype(&::WindowsCreateStringReference) {
    const HMODULE handle =
        ::LoadLibraryEx(L"combase.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (handle) {
      return reinterpret_cast<decltype(&::WindowsCreateStringReference)>(
          ::GetProcAddress(handle, "WindowsCreateStringReference"));
    }
    return nullptr;
  }();
  return create_string_reference_func;
}

}  // namespace

namespace win {

HStringReference::HStringReference(const wchar_t* str, size_t length) {
  // String must be null terminated for WindowsCreateStringReference.
  // nullptr str is OK so long as the length is 0.
  DCHECK(str ? str[length] == L'\0' : length == 0);
  auto create_string_reference_func = GetWindowsCreateStringReference();
  // Windows 7 backport: leaves the instance holding a null HSTRING, which is
  // what every WinRT call this feeds into already treats as a failure.
  if (!create_string_reference_func)
    return;
  const HRESULT hr = create_string_reference_func(
      str, checked_cast<UINT32>(length), &hstring_header_, &hstring_);
  DCHECK_EQ(hr, S_OK);
}

HStringReference::HStringReference(const wchar_t* str)
    : HStringReference(str, str ? wcslen(str) : 0) {}

}  // namespace win
}  // namespace base
