...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Create(Mode mode,
  ...
  std::u16string name;
  // Windows 7 backport: restored from Chromium 109.0.5414.120. Windows before
  // 8.1 ignores the DACL on certain unnamed objects, shared sections among
  // them, so the empty DACL set above does not actually make a read-only
  // region read-only: the handle can still be duplicated with FILE_MAP_WRITE.
  // CheckPlatformHandlePermissionsCorrespondToMode() detects exactly that
  // mismatch and the CHECK in Take() turns it into a breakpoint, killing the
  // browser process the first time a region is handed over. Giving the section
  // a random name makes Windows honour the DACL again. Nothing changes on
  // Windows 8.1 and later, where the sections stay unnamed as before.
