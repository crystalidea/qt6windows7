...
bool Process::SetPriority(Priority priority) {
  ...
  if (base::win::OSInfo::GetInstance()->version() >=
    ...
    // Windows 7 backport: SetProcessInformation() was introduced in Windows 8.
    // The branch is already gated on Windows 11 or later, so resolving it at
    // run time only keeps the import out of the load-time table.
    const auto set_process_information_fn =
        reinterpret_cast<decltype(&::SetProcessInformation)>(
            ::GetProcAddress(::GetModuleHandle(L"kernel32.dll"),
                             "SetProcessInformation"));
    bool ret = set_process_information_fn &&
