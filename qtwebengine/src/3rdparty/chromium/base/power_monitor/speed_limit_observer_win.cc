...
// Windows 7 backport: with current Windows SDKs CallNtPowerInformation() is
// imported from api-ms-win-power-base-l1-1-0.dll, an API set that does not
// exist on Windows 7 and that keeps the whole library from loading there. The
// function itself has been exported by powrprof.dll since Windows XP, so load
// it from there at run time; this is the very same implementation the API set
// forwards to on newer Windows.
NTSTATUS CallNtPowerInformationDynamic(POWER_INFORMATION_LEVEL information_level,
                                       PVOID input_buffer,
                                       ULONG input_buffer_length,
                                       PVOID output_buffer,
                                       ULONG output_buffer_length) {
  using CallNtPowerInformationFn = NTSTATUS(WINAPI*)(POWER_INFORMATION_LEVEL,
                                                     PVOID, ULONG, PVOID,
                                                     ULONG);
  static const CallNtPowerInformationFn call_nt_power_information_fn = []() {
    HMODULE powrprof =
  ...
  if (!call_nt_power_information_fn)
    return static_cast<NTSTATUS>(0xC00000BBL);  // STATUS_NOT_SUPPORTED
  return call_nt_power_information_fn(information_level, input_buffer,
...
bool GetCPUIdleness(int* idleness_percent) {
  ...
  if (!NT_SUCCESS(CallNtPowerInformationDynamic(
          SystemPowerInformation, nullptr, 0, info.get(),
