...
DesktopVector GetDpiForMonitor(HMONITOR monitor) {
  UINT dpi_x, dpi_y;
  // Windows 7 backport: ::GetDpiForMonitor() lives in shcore.dll, which only
  // exists from Windows 8.1 on and whose import keeps the library from loading
  // on Windows 7. Resolve it at run time and fall through to the system DPI
  // below when it is absent - on Windows 7 every monitor uses the system DPI,
  // so that is the correct answer rather than an approximation.
  using GetDpiForMonitorFn = HRESULT(WINAPI*)(HMONITOR, MONITOR_DPI_TYPE,
                                              UINT*, UINT*);
  static const GetDpiForMonitorFn get_dpi_for_monitor_fn =
      []() -> GetDpiForMonitorFn {
    HMODULE shcore =
  ...
  HRESULT hr = get_dpi_for_monitor_fn
                   ? get_dpi_for_monitor_fn(monitor, MDT_EFFECTIVE_DPI, &dpi_x,
                                            &dpi_y)
                   : E_NOTIMPL;
  if (SUCCEEDED(hr)) {
