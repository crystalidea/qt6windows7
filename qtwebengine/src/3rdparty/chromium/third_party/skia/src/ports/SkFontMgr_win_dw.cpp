    ...
    SkUnichar character) const
{
    ...
    // Windows 7 backport: IDWriteFontFallback arrived with Windows 8.1, so on
    // Windows 7 fFontFallback is always null and every character that needs a
    // fallback font used to land in layoutFallback() - which, as the comment in
    // that function says, cannot be stopped from using the system font
    // collection. A sandboxed renderer cannot reach that collection, and
    // DirectWrite answers by throwing a C++ exception rather than returning a
    // failing HRESULT. Nothing in Chromium catches it, so msvcrt - which
    // DWrite.dll is linked against, not the ucrtbase everything else uses -
    // terminates the process with exit code 3 the moment a page lays out text.
    //
    // Chromium avoided this until 109.0.5414.120 by telling Blink not to use
    // Skia's font fallback at all below Windows 8.1; that API was deleted along
    // with Windows 7 support, so report "no match" instead and let Blink fall
    // back on its own logic, which is what the old flag arranged for.
    return nullptr;
}
