...
void SystemSnapshotWin::CPUFrequency(uint64_t* current_hz,
  ...
  // imported from api-ms-win-power-base-l1-1-0.dll, an API set absent on
  // Windows 7. powrprof.dll has exported the function since Windows XP and is
  // what the API set forwards to, so resolve it there at run time instead.
  using CallNtPowerInformationFn = NTSTATUS(WINAPI*)(
  ...
  if (call_nt_power_information_fn(
          ...
          0,
          &info[0],
          sizeof(PROCESSOR_POWER_INFORMATION) * num_cpus) != 0) {
    *current_hz = 0;
    *max_hz = 0;
    return;
  }
