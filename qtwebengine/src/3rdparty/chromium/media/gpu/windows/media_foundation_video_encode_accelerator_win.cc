...
// Windows 7 backport: MFCreateDXGISurfaceBuffer() was introduced in Windows 8.
// mfplat.dll exists on Windows 7, so the delay-load resolves the module and
// then faults on the missing export; resolve it at run time and return a plain
// failure HRESULT, which the caller already handles.
HRESULT CreateDXGISurfaceBufferDynamic(REFIID riid,
                                       IUnknown* surface,
                                       UINT subresource_index,
                                       BOOL bottom_up_when_linear,
                                       IMFMediaBuffer** buffer) {
  using MFCreateDXGISurfaceBufferFn =
      HRESULT(STDAPICALLTYPE*)(REFIID, IUnknown*, UINT, BOOL,
                               IMFMediaBuffer**);
  static const MFCreateDXGISurfaceBufferFn create_dxgi_surface_buffer_fn =
      []() -> MFCreateDXGISurfaceBufferFn {
    HMODULE mfplat =
        ::LoadLibraryEx(L"mfplat.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    return mfplat ? reinterpret_cast<MFCreateDXGISurfaceBufferFn>(
                        ::GetProcAddress(mfplat, "MFCreateDXGISurfaceBuffer"))
                  : nullptr;
  }();
  if (!create_dxgi_surface_buffer_fn)
    return E_NOTIMPL;
  return create_dxgi_surface_buffer_fn(riid, surface, subresource_index,
                                       bottom_up_when_linear, buffer);
}
...
HRESULT MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBufferGpu(
  ...
  ComMFMediaBuffer input_buffer;
  // Windows 7 backport: MFCreateDXGISurfaceBuffer() only exists from Windows 8
  // on; resolved at run time so the delay-load does not fault, and reported as
  // a plain HRESULT failure, which this path already handles by falling back to
  // software encoding.
  hr = CreateDXGISurfaceBufferDynamic(__uuidof(ID3D11Texture2D),
