...
// Windows 7 backport: MFLockDXGIDeviceManager()/MFUnlockDXGIDeviceManager()
// were introduced in Windows 8. mfplat.dll itself is present on Windows 7, so
// the delay-load finds the module and then faults on the missing export.
// Resolve them at run time and report failure instead; InitializeDXGIDeviceManager()
// propagates that HRESULT and the renderer falls back to a non-DXGI path.
FARPROC LoadMFPlatFunctionForRenderer(const char* function_name) {
  static HMODULE const handle =
      ::LoadLibraryEx(L"mfplat.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
  return handle ? ::GetProcAddress(handle, function_name) : nullptr;
}

HRESULT LockDXGIDeviceManagerDynamic(UINT* reset_token,
                                     IMFDXGIDeviceManager** device_manager) {
  using MFLockDXGIDeviceManagerFn =
      HRESULT(STDAPICALLTYPE*)(UINT*, IMFDXGIDeviceManager**);
  static const auto lock_dxgi_device_manager_fn =
      reinterpret_cast<MFLockDXGIDeviceManagerFn>(
          LoadMFPlatFunctionForRenderer("MFLockDXGIDeviceManager"));
  if (!lock_dxgi_device_manager_fn)
    return E_NOTIMPL;
  return lock_dxgi_device_manager_fn(reset_token, device_manager);
}

void UnlockDXGIDeviceManagerDynamic() {
  using MFUnlockDXGIDeviceManagerFn = HRESULT(STDAPICALLTYPE*)();
  static const auto unlock_dxgi_device_manager_fn =
      reinterpret_cast<MFUnlockDXGIDeviceManagerFn>(
          LoadMFPlatFunctionForRenderer("MFUnlockDXGIDeviceManager"));
  if (unlock_dxgi_device_manager_fn)
    unlock_dxgi_device_manager_fn();
}
...
MediaFoundationRenderer::~MediaFoundationRenderer() {
  ...
  if (dxgi_device_manager_) {
    ...
    UnlockDXGIDeviceManagerDynamic();
  }
...
HRESULT MediaFoundationRenderer::InitializeDXGIDeviceManager() {
  ...
  RETURN_IF_FAILED(
      LockDXGIDeviceManagerDynamic(&device_reset_token, &dxgi_device_manager_));
