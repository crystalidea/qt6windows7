...
// Windows 7 backport: MFCreateDXGIDeviceManager() was introduced in Windows 8.
// mfplat.dll itself exists on Windows 7, so the delay-load resolves the module
// and then faults on the missing export. Resolve it by hand and report the
// manager as unavailable instead, which every caller already handles - capture
// then delivers CPU frames and hardware encoding is simply not offered.
HRESULT CreateDXGIDeviceManagerDynamic(UINT* reset_token,
                                       IMFDXGIDeviceManager** device_manager) {
  using MFCreateDXGIDeviceManagerFn =
      HRESULT(STDAPICALLTYPE*)(UINT*, IMFDXGIDeviceManager**);
  static const MFCreateDXGIDeviceManagerFn create_dxgi_device_manager_fn =
      []() -> MFCreateDXGIDeviceManagerFn {
    HMODULE mfplat =
        ::LoadLibraryEx(L"mfplat.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    return mfplat ? reinterpret_cast<MFCreateDXGIDeviceManagerFn>(
                        ::GetProcAddress(mfplat, "MFCreateDXGIDeviceManager"))
                  : nullptr;
  }();
  if (!create_dxgi_device_manager_fn)
    return E_NOTIMPL;
  return create_dxgi_device_manager_fn(reset_token, device_manager);
}
...
scoped_refptr<DXGIDeviceManager> DXGIDeviceManager::Create(CHROME_LUID luid) {
  ...
  HRESULT hr = CreateDXGIDeviceManagerDynamic(&d3d_device_reset_token,
