...
// Windows 7 backport: CM_Get_DevNode_PropertyW() was introduced in Windows 8,
// and importing it keeps the whole library from loading on Windows 7. Resolve
// it at run time; this file is only reached through the MidiManagerWinrt
// backend, which is behind a feature disabled by default and is not used on
// Windows 7 (the winmm-based MidiManagerWin is), so returning a failing
// CONFIGRET simply leaves the driver strings empty there.
CONFIGRET GetDevNodePropertyDynamic(DEVINST dev_inst,
                                    const DEVPROPKEY* property_key,
                                    DEVPROPTYPE* property_type,
                                    PBYTE property_buffer,
                                    PULONG property_buffer_size,
                                    ULONG flags) {
  using CMGetDevNodePropertyWFn = CONFIGRET(WINAPI*)(DEVINST,
                                                     const DEVPROPKEY*,
                                                     DEVPROPTYPE*,
                                                     PBYTE,
                                                     PULONG,
                                                     ULONG);
  static const CMGetDevNodePropertyWFn get_dev_node_property_fn =
      []() -> CMGetDevNodePropertyWFn {
    HMODULE cfgmgr32 =
        ::LoadLibraryEx(L"cfgmgr32.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    return cfgmgr32 ? reinterpret_cast<CMGetDevNodePropertyWFn>(
                          ::GetProcAddress(cfgmgr32, "CM_Get_DevNode_PropertyW"))
                    : nullptr;
  }();
  if (!get_dev_node_property_fn)
    return CR_FAILURE;
  return get_dev_node_property_fn(dev_inst, property_key, property_type,
...
void GetDevPropString(DEVINST handle,
  ...
  CONFIGRET cr = GetDevNodePropertyDynamic(handle, devprop_key, &devprop_type,
  ...
  cr = GetDevNodePropertyDynamic(handle, devprop_key, &devprop_type,
