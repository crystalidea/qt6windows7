...
// Windows 7 backport: the AppContainer profile functions were introduced in
// Windows 8 and importing them keeps the library from loading on Windows 7.
// Resolve them at run time; when they are missing every entry point below
// reports failure, which is the same outcome the callers already get from
// sandbox::features::IsAppContainerSandboxSupported() returning false.
FARPROC LoadUserenvFunctionForAppContainer(const char* function_name) {
  static HMODULE const handle =
      ::LoadLibraryEx(L"userenv.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
  return handle ? ::GetProcAddress(handle, function_name) : nullptr;
}

HRESULT CreateAppContainerProfileDynamic(PCWSTR container_name,
                                         PCWSTR display_name,
                                         PCWSTR description,
                                         PSID_AND_ATTRIBUTES capabilities,
                                         DWORD capability_count,
                                         PSID* container_sid) {
  static const auto create_app_container_profile_fn =
      reinterpret_cast<decltype(&::CreateAppContainerProfile)>(
          LoadUserenvFunctionForAppContainer("CreateAppContainerProfile"));
  if (!create_app_container_profile_fn)
    return E_NOTIMPL;
  return create_app_container_profile_fn(container_name, display_name,
                                         description, capabilities,
                                         capability_count, container_sid);
}

HRESULT DeriveAppContainerSidFromAppContainerNameDynamic(
    PCWSTR container_name,
    PSID* container_sid) {
  static const auto derive_app_container_sid_fn = reinterpret_cast<
      decltype(&::DeriveAppContainerSidFromAppContainerName)>(
      LoadUserenvFunctionForAppContainer(
          "DeriveAppContainerSidFromAppContainerName"));
  if (!derive_app_container_sid_fn)
    return E_NOTIMPL;
  return derive_app_container_sid_fn(container_name, container_sid);
}

HRESULT DeleteAppContainerProfileDynamic(PCWSTR container_name) {
  static const auto delete_app_container_profile_fn =
      reinterpret_cast<decltype(&::DeleteAppContainerProfile)>(
          LoadUserenvFunctionForAppContainer("DeleteAppContainerProfile"));
  if (!delete_app_container_profile_fn)
    return E_NOTIMPL;
  return delete_app_container_profile_fn(container_name);
}
...
AppContainerBase* AppContainerBase::CreateProfile(const wchar_t* package_name,
  ...
  HRESULT hr = CreateAppContainerProfileDynamic(
...
AppContainerBase* AppContainerBase::Open(const wchar_t* package_name) {
  PSID package_sid_ptr = nullptr;
  HRESULT hr = DeriveAppContainerSidFromAppContainerNameDynamic(
      package_name, &package_sid_ptr);
  if (FAILED(hr))
    return nullptr;
...
bool AppContainerBase::Delete(const wchar_t* package_name) {
  return SUCCEEDED(DeleteAppContainerProfileDynamic(package_name));
}
