...
// Windows 7 backport: the version check below is restored from Chromium
// 109.0.5414.120. Disconnecting csrss means closing the ALPC port handle to it,
// and to leave the process in a usable state afterwards the sandbox first
// destroys the heap it shares with csrss - which it locates by walking the
// undocumented _HEAP structure in sandbox/win/src/heap_helper.cc at offsets
// that only match Windows 8 and later. On Windows 7 that search finds nothing,
// CloseOpenHandles() fails and the target kills itself with
// SBOX_FATAL_CLOSEHANDLES (exit code 7010), so no renderer can ever start.
// Below Windows 10 the sandbox simply keeps csrss connected, exactly as
// Chromium did while it still supported these systems.
