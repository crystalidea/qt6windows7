...
// Windows 7 backport: restored from Chromium 109.0.5414.120, where it was the
// path taken on Windows 7 and 8.0. The information class the function below
// uses does not exist there, so the handle table is walked by hand instead:
// handle values are always a multiple of four, so try them in order until as
// many live handles have been found as the process reports, giving up after a
// long enough run of invalid ones to conclude the table has ended.
static std::optional<ProcessHandleMap> GetCurrentProcessHandlesByScanning() {
  DWORD handle_count;
...
std::optional<ProcessHandleMap> GetCurrentProcessHandles() {
  ...
  if (!NT_SUCCESS(status)) {
    // Windows 7 backport: ProcessHandleTable is only understood from
    // Windows 8.1 on; below that the call comes back with
    // STATUS_INVALID_INFO_CLASS. Returning nothing here makes
    // HandleCloserAgent::CloseHandles() fail, which makes the target terminate
    // itself with SBOX_FATAL_CLOSEHANDLES (exit code 7010) - so no renderer
    // ever starts. Fall back to walking the handle table by hand, as Chromium
    // did for these systems until 109.0.5414.120.
    std::optional<ProcessHandleMap> scanned = GetCurrentProcessHandlesByScanning();
    if (scanned)
      return scanned;
