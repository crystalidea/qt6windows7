...
ResultCode TargetProcess::Create(
  ...
  // Windows 7 backport: Windows 8 implements nested jobs, but for older systems
  // we need to break out of any job we're in to enforce our restrictions.
  // Restored from Chromium 109.0.5414.120.
  if (startup_info_helper->HasJobsToAssociate() &&
  ...
  // Windows 7 backport: on Windows 10 and later the job is attached through
  // PROC_THREAD_ATTRIBUTE_JOB_LIST when the process is created. That attribute
  // does not exist below Windows 10, so assign the still-suspended target to
  // the job by hand, as Chromium did up to 109.0.5414.120.
  if (startup_info_helper->HasJobsToAssociate() &&
      !startup_info_helper->JobsAssociatedViaAttribute()) {
    if (!::AssignProcessToJobObject(startup_info_helper->GetJobToAssociate(),
