...
DWORD StartupInformationHelper::CountAttributes() {
  ...
  // Windows 7 backport: the job list attribute is only available from
  // Windows 10 on; below that the job is assigned after process creation.
...
bool StartupInformationHelper::BuildStartupInformation() {
  ...
  // Windows 7 backport: PROC_THREAD_ATTRIBUTE_JOB_LIST was introduced in
  // Windows 10. Where it does not exist the attribute is left out and
  // TargetProcess::Create() calls AssignProcessToJobObject() on the suspended
  // target instead, which is what Chromium did up to 109.0.5414.120.
  if (!job_handle_list_.empty() &&
      base::win::GetVersion() >= base::win::Version::WIN10) {
    if (!startup_info_.UpdateProcThreadAttribute(
    ...
    jobs_associated_via_attribute_ = true;
    expected_attributes--;
  }
