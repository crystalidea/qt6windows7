...
class StartupInformationHelper {
 public:
  ...
  // Windows 7 backport: PROC_THREAD_ATTRIBUTE_JOB_LIST only exists from
  // Windows 10 on. Where it is unavailable BuildStartupInformation() leaves the
  // attribute out and TargetProcess::Create() assigns the job by hand instead,
  // which is how Chromium did it up to 109.0.5414.120. False until
  // BuildStartupInformation() has run.
  bool JobsAssociatedViaAttribute() { return jobs_associated_via_attribute_; }
  // The job the target must be assigned to when the attribute was not used.
  HANDLE GetJobToAssociate() {
    return job_handle_list_.empty() ? nullptr : job_handle_list_[0];
  }
 ...
 private:
  ...
  bool jobs_associated_via_attribute_ = false;
