...
// Windows 7 backport: these entry points were all introduced in Windows 8 and
// are resolved at run time, as Chromium did up to 109.0.5414.120. On Windows 7
// none of the mitigations they configure exist, so the sandbox falls back to
// the restricted token, job object, alternate desktop and integrity level -
// exactly the profile Chromium used there.
...
// Windows 7 backport: on 32-bit Windows 7 the process creation attribute only
// takes a DWORD-sized mitigation mask; passing the 64-bit one makes
// UpdateProcThreadAttribute() fail and no child process starts at all. Restored
// from Chromium 109.0.5414.120.
#if defined(_WIN64)
