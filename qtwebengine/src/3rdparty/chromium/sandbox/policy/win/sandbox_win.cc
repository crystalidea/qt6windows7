...
ResultCode AddDefaultConfigForSandboxedProcess(TargetConfig* config) {
  ...
  // Windows 7 backport: Win8+ adds a device DeviceApi that we don't need. It
  // does not exist on Windows 7, so asking the handle closer to look for it
  // only costs it work.
...
ResultCode GenerateConfigForSandboxedProcess(const base::CommandLine& cmd_line,
  ...
  if (process_type == switches::kRendererProcess) {
    // Windows 7 backport: closing this handle is only safe when the renderer
    // draws its randomness from bcryptprimitives!ProcessPrng, which exists from
    // Windows 10 on. Below that both base::RandBytes() and BoringSSL fall back
    // to advapi32!RtlGenRandom, forwarded to cryptbase.dll, which opens
    // \Device\KsecDD once during sandbox warmup and keeps that handle for the
    // life of the process. Closing it right before lockdown makes every later
    // call fail, and BoringSSL answers a failing CRYPTO_sysrand() with abort():
    // the renderer dies silently with exit code 3 the moment the page asks for
    // random bytes.
    if (base::win::GetVersion() >= base::win::Version::WIN10 &&
        base::FeatureList::IsEnabled(features::kWinSboxRendererCloseKsecDD)) {
...
ResultCode SandboxWin::AddWin32kLockdownPolicy(TargetConfig* config) {
  ...
  // Windows 7 backport: win32k lockdown is a Windows 8 process mitigation, and
  // Chromium gated this whole function on it until the version check was
  // dropped along with Windows 7 support. Without the gate a Windows 7 renderer
  // asks for a mitigation the kernel cannot apply and, more importantly, turns
  // on the sandbox's fake GDI initialisation - a path that only ever ran in
  // processes where win32k really was locked down.
  if (base::win::GetVersion() < base::win::Version::WIN8)
    return SBOX_ALL_OK;
