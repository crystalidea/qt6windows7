...
// Windows 7 backport: CompareObjectHandles() is a Windows 10 API with no
// Windows 7 equivalent, and importing it through
// api-ms-win-core-handle-l1-1-0.dll keeps the library from loading there.
// Resolve it at run time; where it is missing this consistency check between a
// token and its already-registered handle cannot be made, so it is skipped
// rather than turned into a failure. The surrounding code needs NT handles for
// D3D11 shared resources, which are Windows 8 and later, so this is unreachable
// on Windows 7 in practice.
bool CompareObjectHandlesOrAssumeSame(HANDLE first, HANDLE second) {
  using CompareObjectHandlesFn = BOOL(WINAPI*)(HANDLE, HANDLE);
  static const CompareObjectHandlesFn compare_object_handles_fn =
      []() -> CompareObjectHandlesFn {
    HMODULE kernelbase = ::GetModuleHandle(L"kernelbase.dll");
    return kernelbase ? reinterpret_cast<CompareObjectHandlesFn>(
                            ::GetProcAddress(kernelbase, "CompareObjectHandles"))
                      : nullptr;
  }();
  if (!compare_object_handles_fn)
    return true;
  return !!compare_object_handles_fn(first, second);
}
...
DXGISharedHandleManager::GetOrCreateSharedHandleState(
  ...
  if (it != shared_handle_state_map_.end()) {
    ...
    if (!CompareObjectHandlesOrAssumeSame(shared_handle.Get(),
                                          state->GetSharedHandle())) {
