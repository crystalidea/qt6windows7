...
void DWriteFontProxyImpl::InitializeDirectWrite() {
  ...
  // Windows 7 backport: the comments and DCHECKs that used to be here claimed
  // both queries must succeed "since we only support >= Win10". They do not on
  // Windows 7: IDWriteFactory2 arrived with Windows 8.1 and IDWriteFactory3
  // with Windows 10. DCHECK compiles out in release builds, so the empty
  // factory3_ went straight into GetLocalFontCollection() below and faulted on
  // its first virtual call, taking the browser process down as soon as a page
  // needed fonts. The wording below is the one Chromium used while it still
  // supported these systems, up to 109.0.5414.120.
  ...
  if (factory3_) {
    ...
    // Windows 7 backport: no font set API here, so ask the base factory
    // directly - which is exactly what Chromium did before sideloading was
    // added, and all that side loading is used for is testing.
