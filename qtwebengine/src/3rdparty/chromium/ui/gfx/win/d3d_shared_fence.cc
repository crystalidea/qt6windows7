...
bool D3DSharedFence::IsSameFenceAsHandle(HANDLE shared_handle) const {
  // Windows 7 backport: CompareObjectHandles() is a Windows 10 API (there is no
  // Windows 7 equivalent - it wraps NtCompareObjects, which that kernel does not
  // have). Resolve it at run time so the import does not keep the library from
  // loading. Where it is missing, fall back to comparing the handle values: two
  // equal handles in one process always denote the same object, and the only
  // cost of the false negative is that a fence object is not reused. This path
  // is unreachable on Windows 7 anyway, since D3D11 fences require Windows 10.
  ...
  if (!compare_object_handles_fn)
    return shared_handle_.Get() == shared_handle;
