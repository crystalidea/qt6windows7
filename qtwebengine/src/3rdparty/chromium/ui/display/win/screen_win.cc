...
absl::optional<int> GetPerMonitorDPI(HMONITOR monitor) {
  UINT dpi_x, dpi_y;
  // Windows 7 backport: ::GetDpiForMonitor() is exported by shcore.dll, which
  // only exists from Windows 8.1 on. Resolve it at run time and report "no
  // per-monitor DPI" when it is missing - Windows 7 has no per-monitor DPI at
  // all, and the callers already fall back to the global scale factor.
  ...
  if (!get_dpi_for_monitor_fn)
    return absl::nullopt;
  if (!SUCCEEDED(
          get_dpi_for_monitor_fn(monitor, MDT_EFFECTIVE_DPI, &dpi_x, &dpi_y))) {
    return absl::nullopt;
  }
